...
public class Scheduler extends Thread {
    ...
    private List<SchedulingPolicy> schedulingPolicies; // 每个队列的调度策略
    ...
    public Scheduler() {
        ...
        // 初始化调度策略列表（默认为空）
        schedulingPolicies = new ArrayList<>(4);
        for (int i = 0; i < 4; i++) {
            schedulingPolicies.add(null); // 初始化为空
        }
    ...
    // 设置队列的调度策略
    public void setQueuePolicy(int queueIndex, SchedulingPolicy policy) {
        if (queueIndex >= 0 && queueIndex < 4) {
            schedulingPolicies.set(queueIndex, policy);
            System.out.println("队列 " + queueIndex + " 的调度策略设置为 " + policy);
        } else {
            System.out.println("无效的队列索引: " + queueIndex);
        }
    }
    ...
    public PCB getNextProcess() {
        ...
        try {
            ...
            for (int i = 0; i < readyQueues.size(); i++) {
                ...
                if (!queue.isEmpty()) {
                    PCB pcb = selectProcessFromQueue(queue, schedulingPolicies.get(i));
                    if (pcb != null) {
                        System.out.println("调度器选择进程 " + pcb.getPid() + "，优先级为 " + i + "，调度策略为 " + schedulingPolicies.get(i));
                        return pcb;
                    }
                }
            }
    ...
    private PCB selectProcessFromQueue(BlockingQueue<PCB> queue, SchedulingPolicy policy) {
        switch (policy) {
            case FCFS:
                return queue.poll(); // 先来先服务，直接取队首进程
            case SJF:
                return selectShortestJob(queue); // 最短作业优先
            case RR:
                return queue.poll(); // 时间片轮转，直接取队首进程
            case PRIORITY:
                return selectHighestPriority(queue); // 优先级调度
            default:
                return queue.poll(); // 默认使用FCFS
        }
    }

    // 最短作业优先（SJF）：选择剩余时间最短的进程
    private PCB selectShortestJob(BlockingQueue<PCB> queue) {
        PCB shortestJob = null;
        int shortestTime = Integer.MAX_VALUE;

        // 遍历队列，找到剩余时间最短的进程
        for (PCB pcb : queue) {
            int remainingTime = pcb.getTimeSlice() - pcb.getTimeUsed();
    ...
    // 优先级调度：选择优先级最高的进程
    private PCB selectHighestPriority(BlockingQueue<PCB> queue) {
        ...
        if (highestPriorityPCB != null) {
            queue.remove(highestPriorityPCB);
        }
